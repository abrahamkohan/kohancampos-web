...
function formatPrecio(desde: number | null, hasta: number | null, moneda: string) {
  if (!desde) return null
  const fmt = (n: number) => `${moneda} ${n.toLocaleString("es-PY")}`
...
export default async function ProyectoDetallePage({
  ...
  const precio    = formatPrecio(p.precio_desde, p.precio_hasta, p.moneda)
          ...
          <div className="mx-auto max-w-[1100px]">
            ...
            <div className="grid md:grid-cols-[1fr_360px] gap-10 items-start">
              ...
              <div className="flex flex-col gap-5 md:sticky md:top-28">
                ...
                <div className="flex flex-col gap-2.5 border-t border-gold/10 pt-4">
                  {p.ubicacion && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-sans text-sm font-[300] text-kc-white/60">{p.ubicacion}</span>
                    </div>
                  )}
                  {p.delivery_date && (
                    <div className="flex items-center gap-2">
                  ...
                  {precio && (
                    <div className="flex items-center gap-2">
                      <span className="font-sans text-sm text-gold font-[400]">{precio}</span>
                    </div>
                  )}
                </div>
                ...
                {p.descripcion && (
                  <p className="font-sans text-sm font-[300] text-kc-white/55 leading-relaxed border-t border-gold/10 pt-4">
                    {p.descripcion}
                  </p>
                )}
              </div>
