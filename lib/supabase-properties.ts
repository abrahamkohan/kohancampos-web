...
export async function getPropiedadById(id: string): Promise<PropiedadDetalle | null> {
  const [propRes, fotosRes] = await Promise.all([
    fetch(
      SUPABASE_URL + "/rest/v1/properties?id=eq." + id + "&select=id,titulo,tipo,ciudad,barrio,zona,direccion,precio,moneda,financiacion,amoblado,superficie_m2,superficie_cubierta_m2,terreno_m2,dormitorios,banos,garajes,piso,condicion,deposito,descripcion,operacion,foto_portada,latitud,longitud&limit=1",
